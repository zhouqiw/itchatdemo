...
def getinfo(id):
    ...
    print url + str(id)
    try:
        result =  requests.get(url+str(id),headers = headers,timeout=2).text
    ...
    # print rs["code"]
    if rs["code"] == 0:
        return rs["data"]["card_number"],rs["data"]["balance_time"],rs["data"]["card_balance"].replace('元', '')
